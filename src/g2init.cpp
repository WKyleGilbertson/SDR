...
#include "G2INIT.h"
  ...
  const unsigned short GPSDelays[37] = {5, 6, 7, 8, 17, 18, 139, 140, 141, 251, 252,
  254, 255, 256, 257, 258, 469, 470, 471, 472, 473, 474, 509, 512, 513, 514,
  515, 516, 859, 860, 861, 862, 863, 950, 947, 948, 950};
  const unsigned short GPSF10[37] = {01440, 01620, 01710, 01744, 01133, 01455,
  ...
  const unsigned short SBASDelays[39] = {145, 175, 52, 21, 237, 235, 886, 657, 634,
    762, 355, 1012, 176, 603, 130, 359, 595, 68, 386, 797, 456, 499, 883, 307,
    127, 211, 121, 118, 163, 628, 853, 484, 289, 811, 202, 1021, 463, 568, 904};
  const unsigned short SBASF10[39] = {00671, 00536, 01510, 01545, 00160, 00701,
...
int G2INIT::PRNGEN(uint8_t * CACODE_OUT, uint8_t PRN, uint16_t CODEPHASE) {
  ...
  unsigned short G2=0x0000;
  int8_t CA[1023];
  uint8_t temp_ca[1023], chip;
  uint16_t g1_fb, g2_fb, idx = 0;
//  unsigned short F10Chips = 0x0000, F10Inverse = 0x0000, CAreg = 0;

// 1. G2 Init Logic
if (PRN < 40) {
  G2 = flip(GPSF10[PRN-1]^0x3FF);
} else {
  G2 = flip(SBASF10[PRN-120]^0x3FF);
}
//  printf("G2Init: o%.4o F10: o%.4o \n", G2, flip(G2)^0x3FF);

// 2. Generate CA code and store states
 for (idx=0; idx<1023; idx++) {
    G1a[idx] = G1;
    G2a[idx] = G2;

    chip = ((G1 & TAP10) ^ (G2 & TAP10))  ? 1 : 0;
    temp_ca[idx] = chip;
    // Advance G1
    g1_fb = ((G1 & TAP3) >> 7) ^ (G1 & TAP10);
    G1 = (G1 >> 1) | (g1_fb << 9);
    // Advance G2
    g2_fb = ((G2 & TAP2) >> 8) ^ ((G2 & TAP3) >> 7) ^ ((G2 & TAP6) >> 4) ^
           ((G2 & TAP8) >> 2) ^ ((G2 & TAP9) >> 1) ^ (G2 & TAP10);
    G2 = (G2 >> 1) | (g2_fb << 9);
 }
// 3. Rotation of CA code by CODEPHASE
  for (idx=0; idx<1023; idx++) {
    CACODE[idx] = temp_ca[(idx+CODEPHASE) % 1023];
  }
//4. Verification variables
F10 = 0; F10I = 0;
for (idx=0; idx<10; idx++) {
  F10 |= (temp_ca[idx] << (9-idx));
  F10I |= (temp_ca[idx] << idx);  
}
  /*
  ...
  F10I = F10Inverse; */
   /*
  F10Inverse e= (CA[0]<<9) | (CA[1]<<8) | (CA[2]<<7) | (CA[3]<<6) | (CA[4]<<5) |
           (CA[5]<<4) | (CA[6]<<3) | (CA[7]<<2) | (CA[8]<<1) | CA[9];
