...
CorrelatorResult ChannelProcessor::Correlator(const RawSample *samples, size_t availableSamples) {
    ...
    // Loop through our raw input sample pointer sequentially
    for (i = 0; i < availableSamples; ++i) {
        ...
        int16_t bb_i = (int16_t)(samples[i].i * _carrNco.cosine(carrIdx));
        int16_t bb_q = (int16_t)(samples[i].q * _carrNco.sine(carrIdx));
        ...
        if (_codeNco.getRotations() > oldCodeRotations) {
            ...
            if (_msIntegrated >= 1) {
                epochTriggered = true;
                break; // CRITICAL BREAKOUT: Leave leftover fractions untouched for the next 1ms epoch block!
            }
        }
    }
    ...
    if (epochTriggered) {
        ...
        res.consumed_sample_count = i + 1; // Set the exact consumption index offset boundary

        res.rollover_sample_idx = samples[i].sample_tick;
        res.unix_time = samples[i].unix_time;

        float I = (float)_acc.Pi;
        float Q = (float)_acc.Pq;
        float dynamicT = (float)res.consumed_sample_count / (float)_fs;
        
        // Use your stable normalized Costas Cross-Product Discriminator
        ...
        // Apply tracking filter corrections smoothly
        //float carrNcoUpdate = _oldCarrNco + (_carrLF.tau2 / _carrLF.tau1) * (carrError - _oldCarrError) + (carrError * (0.001f / _carrLF.tau1));
        ...
        //float codeNcoUpdate = _oldCodeNco + (_codeLF.tau2 / _codeLF.tau1) * (codeError - _oldCodeError) + (codeError * (0.001f / _codeLF.tau1));
