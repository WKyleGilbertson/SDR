        ...
        for (int s = 0; s < 2; ++s)
        {
            ...
            if (goldCodeEpochEvent)
            {
                ...
                if (_msIntegrated >= target_integration_ms)
                {
                    ...
                    // --- CARRIER LOOP FILTER (PLL) ---
                    // Correct implementation of standard 2nd-order loop filter
                    ...
                    _doppler_hz = (double)carrNcoUpdate;

                    // FIX: Doppler must be ADDED to match standard low-side mixing tracking
                    float finalFreq = _carrFreqBasis - (float)_doppler_hz;
